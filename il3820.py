...
class EPD:
    ...
    def init(self):
        ...
        self._command(0x01, bytearray([(EPD_HEIGHT - 1) & 0xFF, ((EPD_HEIGHT - 1) >> 8) & 0xFF, 0x00])) # Driver output control
        self._command(0x11, bytearray([0x03])) # Data entry mode
        self._command(0x44, bytearray([0x00, (EPD_WIDTH // 8) - 1])) # Set Ram-X address start/end position
        self._command(0x45, bytearray([0x00, 0x00, (EPD_HEIGHT - 1) & 0xFF, ((EPD_HEIGHT - 1) >> 8) & 0xFF])) # Set Ram-Y address start/end position
